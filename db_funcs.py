...
def start():
    ...
    if 'y' in input('Continuing will restart database.sqlite3, proceed? (Y/N): ').lower():
...
def new_post(title: str, content: str, date: datetime.datetime | str) -> tuple[int,str,str,str]:
    ...
    update_inject()
    return cursor.execute("SELECT id,title,content,date,id FROM posts WHERE id=?;" , [id]).fetchone()
...
def update_ip(ip: str, blacklisted: bool) ->  tuple[str, int]:
    ...
    cursor.execute('UPDATE ips SET ip=?, blacklisted=?  WHERE ip=?;', (ip, int(blacklisted), ip))
    update_inject()
    return cursor.execute('SELECT * FROM ips WHERE ip=?', [ip]).fetchone()
...
def update_post(id: int, title: str, content: str, date: datetime.datetime | str) -> tuple[int, str, str, str]:
    '''Updates a post with the arguments provided.'''
    date = date.strftime("%Y-%m-%d")if isinstance(date, datetime.datetime) else date
    cursor.execute('UPDATE posts SET id=?,title=?,content=?,date=? WHERE id=?', [id, title, content, date, id])
    return get_post(id)
